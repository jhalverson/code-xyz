...
int main(int argc, char* argv[]) {
  ...
  particles.readScript(f, step, steps, freqWrite, dt, sidex,sidey,sidez, k_spring, verlet, rc, rs,
  ...
  particles.writeConfiguration(step, end_step, freqWrite, dt, sidex, sidey, sidez, k_spring, bonds, unbreakable, a0);
  cells.init(sidex, sidey, sidez, rc, rs, verlet);
  double rs_next = rs;
  double rs_prev = rs - 0.05;
  particles.buildPairs(pairs, cells, sidex, sidey, sidez, verlet, rs, rc, skin_violation, rs_next);
  particles.updateBondList(pairs, sidex, sidey,sidez, dt, bonds, unbreakable, xcp, Lcp, step, life_times, max_site_type);

  long long t;
  for (t = step + 1; t <= end_step; t++) {
    particles.computeExcludedVolumeForces(pairs, sidex, sidey, sidez, remove_overlap);
//    particles.computeSpringForcesAndTorques(side, bonds, Lcp, k_spring);//not used niladri
  
    particles.computeForcesAndTorques(pairs, sidex, sidey, sidez, max_site_type);
//    particles.computeNonPatchForcesAndTorques(pairs, side, max_site_type);
//    particles.applyRepulsiveForce(sidex,sidey,sidez);
//    particles.applyRepulsiveForce2(sidex,sidey,sidez);



//    if (hydrodynamics) particles.updateDiffusionTensor(pairs, side, a0, Dij);//not used niladri
...
//    if (t % 10000 == 0) particles.writeContactsALL(t, dt, sidex,sidey,sidez);
...
//    if (t % 10000 == 0) particles.writeBondLifeTimes(life_times, dt);
  ...
  particles.writeConfiguration(t - 1, end_step, freqWrite, dt, sidex, sidey, sidez, k_spring, bonds, unbreakable, a0);
