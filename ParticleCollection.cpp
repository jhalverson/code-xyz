...
void mic(double& xij_, double& yij_, double& zij_, const double sidex_,
         const double sidey_, const double sidez_) {
  double side_halfx = 0.5 * sidex_;
  double side_halfy = 0.5 * sidey_;
  double side_halfz = 0.5 * sidez_;
  if(xij_ >  side_halfx) xij_ -= sidex_;
  if(xij_ < -side_halfx) xij_ += sidex_;
  if(yij_ >  side_halfy) yij_ -= sidey_;
  if(yij_ < -side_halfy) yij_ += sidey_;
...
void ParticleCollection::buildPairs(PairList& pairs_, CellCollection& cells_, const double sidex_,  
                                    const double sidey_, const double sidez_,
...
void ParticleCollection::updateBondList(const PairList pairs_, double sidex_, double sidey_, double sidez_, double dt_,
    ...
    for (PairList::const_iterator it = pairs_.begin(); it < pairs_.end(); it++) {
        ...
        SiteCollection& sj = pj.sites;     
         ...
         for (size_t a = 0; a < si.size(); a++) {
          ...
          int type_q = pj.type * pj.type * pj.type + pi.type * pi.type * pi.type; //april 22
          int site_q = sa.type * sa.type * sa.type + sb.type * sb.type * sb.type;
          if ((type_q == 64 && site_q == 64) || (type_q == 125 && site_q == 1729) || (type_q == 216 && site_q == 13832) || (type_q == 343 && site_q == 21979) || (type_q == 65 && site_q == 637) || (type_q == 126 && site_q == 2926) || (type_q == 217 && site_q == 16625) || (type_q == 344 && site_q == 25720) || (type_q == 72 && site_q == 4312) || (type_q == 133 && site_q == 7657) || (type_q == 224 && site_q == 23408) || (type_q == 351 && site_q == 33859) || (type_q == 91 && site_q == 8343) || (type_q == 152 && site_q == 12636) || (type_q == 243 && site_q == 30331) || (type_q == 370 && site_q == 41958)) {
      ...
      if ( type_q == 64  || type_q == 126 || type_q == 224 || type_q == 370 ) 
          { 
     epsiRep = 40.0;
     epsi    = 40.0;
        }
...
for (ParticleCollection::iterator it = this->begin(); it < this->end(); it++) {
  ...
  p.fz -= pow(1.0 / (p.z - z_value),12);
  }
}


void ParticleCollection::computeExcludedVolumeForces(const PairList pairs_, double sidex_,double sidey_,double sidez_,
...
//   int type_sum = pj.type + pi.type;

    int type_q = pj.type * pj.type * pj.type + pi.type * pi.type * pi.type; //april 22
//      if (sa.type + sb.type ==0 ) {
      if (type_q == 0 || type_q== 1 || type_q == 8 || type_q == 27 || type_q == 2 || type_q== 9 || type_q == 28 || type_q == 16 || type_q == 35 || type_q== 54 || type_q == 128 || type_q == 189 || type_q == 280 || type_q== 407 || type_q == 250 || type_q == 341 || type_q == 468 || type_q== 432 || type_q == 559 || type_q == 686 ) {
//      if (type_sum == 0 || type_sum == 2 || type_sum == -2) {
...
void ParticleCollection::updatePositionsAndOrientations(double sidex_,double sidey_, double sidez_, double dt_, int verlet_, double rc_,
...
//    if (p.type != -1 && p.type != 1) {
    ...
    if(p.x < 0.0) {p.x += sidex_; p.xflag -= 1;}
    if(p.y < 0.0) {p.y += sidey_; p.yflag -= 1;}
    if(p.z < 0.0) {p.z += sidez_; p.zflag -= 1;}
    if(p.x > sidex_) {p.x -= sidex_; p.xflag += 1;}
    if(p.y > sidey_) {p.y -= sidey_; p.yflag += 1;}
    if(p.z > sidez_) {p.z -= sidez_; p.zflag += 1;}

    // check for a skin violation
    if (verlet_ > 0 && !skin_violation_) {
      rsq = pow(p.x + p.xflag * sidex_ - p.x_skin, 2) +
            pow(p.y + p.yflag * sidey_ - p.y_skin, 2) +
            pow(p.z + p.zflag * sidez_ - p.z_skin, 2);
  ...
  if (verlet_ > 0 && skin_violation_) {
    for (ParticleCollection::iterator it = this->begin(); it < this->end(); it++) {
      Particle& p = *it;
      p.updatePositionAtPairListBuild(sidex_,sidey_,sidez_);
    }
  }
...
void ParticleCollection::writeConfiguration(long long t_, long long steps_, long long freqWrite_,
  ...
  config << "<boxSidex>" << sidex_ << "</boxSidex>" << std::endl;
  config << "<boxSidey>" << sidey_ << "</boxSidey>" << std::endl;
  config << "<boxSidez>" << sidez_ << "</boxSidez>" << std::endl;
  config << std::endl;
  for (it = this->begin(); it < this->end(); it++) {
    Particle p = *it;
...
void ParticleCollection::readConfiguration(const char* f_, long long& step_, double& sidex_,double& sidey_, 
                                           double& sidez_,  BondList& bonds_, BondList& unbreakable_, 
                                           const double a0_) {
  std::string line;
  std::string tag;
  std::string value;
    ...
    while (fname.good()) {
      ...
      if (line.size() > 0) {
        ...
        if (tag == "<step>") {
          ...
          value = stripValue(line);}
          //dt_ = StringToNumber<double>(value); }
        else if (tag == "<boxSidex>") {
          value = stripValue(line);
          sidex_ = StringToNumber<double>(value); }
        else if (tag == "<boxSidey>") {
          value = stripValue(line);
          sidey_ = StringToNumber<double>(value); }
        else if (tag == "<boxSidez>") {
          value = stripValue(line);
          sidez_ = StringToNumber<double>(value); }
...
void ParticleCollection::printMSD(long long t_, double sidex_, double sidey_, double sidez_, double dt_) const {
  double x_final;
  double y_final;
  double z_final;
  double msd;
  ...
  std::vector<Particle>::const_iterator it;
  for (it = this->begin(); it < this->end(); it++) {
    x_final = it->x + it->xflag * sidex_;
    y_final = it->y + it->yflag * sidey_;
    z_final = it->z + it->zflag * sidez_;
